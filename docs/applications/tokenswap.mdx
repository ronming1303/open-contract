...
| `pricing` | Fixed ticket price, paid in full by the Buyer at match |
...
| Buyer violated allowed use | Private evidence commitment and selectively disclosed envelope | See [Buyer Misuse Complaints](#buyer-misuse-complaints) |
...
- **Evidence**: the same private evidence envelope and selective-disclosure mechanism used elsewhere (see [Usage Receipts](#usage-receipts)) — the Seller discloses only what's needed to support the claim, not the Buyer's full request/response history.
...
This single criterion is evaluated automatically from the final receipt's `availability` value against the ticket's `sla` threshold:

- **`met`** — final `availability` ≥ the ticket's `sla` threshold, with no unresolved `indeterminate` calls and no denial pattern attributable to the Seller (credential never provisioned, readiness receipt never signed, excess `gateway_unavailable`/`provider_error`).
