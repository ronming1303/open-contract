...
sequenceDiagram
    ...
    Note over OC: Automatic evaluation, design only, not yet implemented
...
|---|---|---|---|
| Identity key | Long-term, the Buyer's existing Ethereum key | Signs the API Key challenge, x402 payment authorization, and the Buyer-TEE handshake; also the seed the archival decryption key is derived from | Never leaves the Buyer |
...
| Archival decryption key | Long-term X25519, derived deterministically from the identity key | Decrypts the Buyer's own Part 1 evidence envelope (see [Private evidence envelope](#private-evidence-envelope)) | Private half never leaves the Buyer; public half is registered with the ticket out of band for the TEE to encrypt against |
...
OpenContract issues Bearer API Keys to the Buyer and Seller after verifying a one-time signature from each party's identity key, and stores the public halves that need out-of-band registration on the ticket — the TEE's signing public key and the Buyer's archival decryption public key. It never receives any party's private key, the Provider credential, or Part 1 plaintext.
...
2. OpenContract verifies the TEE signature, measurement, policy hash, and checkpoint continuity, then computes `availability` from the checkpoint history and writes `criteriaMet` immediately — no Buyer action is needed to produce this result.
