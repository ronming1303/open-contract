...
A single view of the whole lifecycle — listing, credential provisioning, the per-call loop, periodic checkpoints, and settlement — with where encryption applies at each hop:
...
sequenceDiagram
    ...
    Note over Seller,OC: Listing and match
    Seller->>OC: Create ticket, worker-initiated, post Worker Stake
    Buyer->>OC: Accept, lock Escrowed Funds at the full price

    Note over Seller,TEE: Credential provisioning, once
    TEE->>TEE: Generate ephemeral keys, boot approved code
    TEE->>Seller: Attestation, measurement, and policy hash
    Seller->>Seller: Verify attestation, measurement, and policy hash
    Seller->>TEE: Encrypted Provider credential, only the TEE can decrypt it

    Note over Buyer,Log: Every call, loops N times
    loop each call
        Buyer->>TEE: Signed request with nonce, timestamp, and ticket ID
        TEE->>TEE: Verify signature, nonce, window, and quota
        TEE->>Provider: Invoke using the Seller's credential, which never leaves the TEE
        Provider->>TEE: Raw response
        TEE->>TEE: Split the response into Part 1 content and Part 2 metadata
        TEE->>Buyer: Plaintext response and signed receipt, in real time
        TEE->>Log: Flush Part 1, encrypted to the Buyer's key only
        TEE->>Log: Flush Part 2, unencrypted status and metadata
    end

    Note over TEE,OC: Periodic checkpoint, not every call
    opt count, time, or value threshold reached
        TEE->>OC: Submit checkpoint with sequence range, merkleRoot, teeSignature, and cumulative counts
        OC->>OC: Verify the sequence is gapless, append one history row
    end

    Note over TEE,OC: Window close
    TEE->>TEE: Destroy the credential, sign the Final Receipt
    TEE->>OC: Submit Delivery, status moves from matched to under-review

    Note over OC: Automatic evaluation at Submit Delivery
    OC->>OC: Read every checkpoint for this contract
    OC->>OC: Sum the failures attributable to the Seller per the fault table
    OC->>OC: Compute availability as success over success plus seller failures
    OC->>OC: Compare availability to the ticket's sla, write criteriaMet

    Note over Buyer,OC: Review window, unchanged from a generic Working Contract
    OC->>Buyer: criteriaMet already computed, nothing subjective to review
    Buyer->>OC: Approve early, dispute, or take no action before the Review Deadline

    alt fully met
        OC->>Seller: Full Ticket Price and returned Stake
    else none met
        OC->>Buyer: Full refund
        OC->>OC: Slash Stake
    end
```
...
| Hop | Encrypted? | Key |
|---|---|---|
| Seller → TEE (credential) | Yes | TEE's attested public key |
| Buyer → TEE (request) | Yes, session-level | TEE's session key |
| TEE → Provider | TLS, terminates inside the TEE | Provider's own certificate |
| TEE → Buyer (response) | Yes in transit; Buyer decrypts | Session key |
| TEE → evidence log, Part 1 (content) | **Yes** | **Buyer's key only** |
| TEE → evidence log, Part 2 (metadata) | **No** | — |
| TEE → OC, checkpoint | **No** (hashes and counts, no content) | — |

Three storage locations, three different contents:

- **Encrypted evidence log** — every call's full record (Part 1 ciphertext + Part 2 plaintext), decoupled from the TEE's own compute instance and from OpenContract's database. See [Evidence Storage and Checkpointing](#evidence-storage-and-checkpointing).
...
| `allowedModels` | Exact model/deployment allowlist. Required and non-empty: the TEE Gateway rejects any request naming a model outside it as `policy_rejected` before it reaches the Provider, so the Buyer's mistake isn't recorded as a Provider failure against the Seller |
| `startsAt`, `endsAt` | Inclusive service window boundaries — distinct from the Working Contract's own deadlines, which govern matching and delivery, not when the Buyer may actually consume capacity. Validated at Create Contract time against those deadlines: `startsAt` must not be before `matchDeadline` and `endsAt` must not be after `deliveryDeadline`, so the window can never open before matching could possibly have happened or stay open past the point delivery is due — at the cost of the Buyer possibly waiting for the window to open if matched early |
...
| Key / credential | Type | Purpose | Custody |
|---|---|---|---|
| Identity key | Long-term, the Buyer's existing Ethereum key | Signs the API Key challenge, x402 payment authorization, and the Buyer-TEE handshake (which also binds the archival public key, below); also the seed the archival decryption key is derived from | Never leaves the Buyer |
| API Key | Bearer token issued by OpenContract | Authenticates the Buyer's own calls to the OpenContract platform API (e.g. Accept Ticket) | Held by the Buyer only — never given to the TEE Gateway |
| Archival decryption key | Long-term X25519, derived deterministically from the identity key | Decrypts the Buyer's own Part 1 evidence envelope (see [Private evidence envelope](#private-evidence-envelope)) | Private half never leaves the Buyer; the public half is bound into the Buyer's signed handshake message, so the TEE encrypts the archive to it and nobody on the path can swap in a different key |
| Session handshake key | Ephemeral X25519, generated fresh for each ticket's service window | Paired with the TEE's session handshake key via ECDH to derive the key that encrypts live request/response traffic during the window | Discarded when the service window closes |
...
sequenceDiagram
    ...
    TEE->>TEE: Generate ephemeral session key
    TEE->>Buyer: Ephemeral public key signed by TEE identity key
    Buyer->>Buyer: Generate ephemeral session key
    Buyer->>TEE: Ephemeral public key signed by Buyer identity key
    Buyer->>Buyer: Derive shared session key
    TEE->>TEE: Derive shared session key
    Note over Buyer,TEE: Session key encrypts live request and response traffic
    TEE->>TEE: Discard session key when the service window closes
```
...
| Key / credential | Type | Purpose | Custody |
|---|---|---|---|
| Identity key | Long-term, the Seller's existing Ethereum key | Signs the API Key challenge, x402 calls, and the Seller-TEE provisioning handshake, so the TEE only accepts sellerApiKey and the Provider credential from this ticket's actual Seller | Never leaves the Seller |
| sellerApiKey | Bearer token issued by OpenContract | Delegated to the TEE Gateway so it can autonomously submit checkpoints and the final receipt under the Seller's identity, without the Seller relaying every call | Generated by the Seller, delivered to the TEE over the provisioning handshake below rather than in the clear; should be scoped to this ticket's checkpoint/delivery endpoints rather than full account privilege |
| Provider credential | The Seller's own upstream credential | Lets the TEE invoke the Upstream Provider on the Seller's behalf | Delivered to the TEE over the same provisioning handshake — see [TEE Attestation and Credential Provisioning](#tee-attestation-and-credential-provisioning) |
| Provisioning handshake key | Ephemeral X25519, generated once when the TEE is provisioned | Paired with the TEE's provisioning handshake key via ECDH to derive the one-time key that encrypts sellerApiKey and the Provider credential in transit | Discarded once the credential is delivered |

### TEE Gateway

| Key / credential | Type | Purpose | Custody |
|---|---|---|---|
| Signing identity key | Ed25519 | Signs receipts, checkpoints, and the TEE's own ephemeral handshake public keys, giving each handshake a verifiable origin | Private half never leaves the TEE; public half is registered with the ticket so Buyer and Seller can verify signatures |
| Buyer-facing handshake key | Ephemeral X25519, generated once per service window | Paired with the Buyer's session handshake key to derive the live-transport session key | Discarded when the service window closes |
| Seller-facing handshake key | Ephemeral X25519, generated once at provisioning | Paired with the Seller's provisioning handshake key to derive the one-time provisioning key | Discarded once credentials are received |
| Decrypted sellerApiKey / Provider credential | Plaintext, held only after the provisioning handshake | Used respectively to call the OpenContract platform API and the Upstream Provider | Held only in the TEE's memory; never re-exported |
...
TokenSwap uses the protocol-generic [Complaints](/core-concepts/working_contracts#complaints) mechanism for this — see [File Complaint](/api-reference/contracts/complaints) for the API. A Seller flagging Buyer misuse is TokenSwap's motivating case, not a bespoke TokenSwap-only feature: either party can file, the same way on any Working Contract.
...
The rest of this page describes the target design. The process-isolated Gateway that exists today implements a subset of it, and differs from it in the ways below. All of it has been exercised end to end against a testnet ticket.

**Implemented**

- Ticket creation, with policy validation: `allowedModels` must be non-empty, and the service window must sit inside the ticket's own deadlines (see [Ticket policy](#ticket-policy-application-metadata)).
- The Seller-TEE provisioning handshake and the Buyer-TEE session handshake, each an ephemeral key exchange endorsed by the initiating party's identity key; `sellerApiKey` and the Provider credential are delivered only through the former, and the Buyer's archival public key is bound into the latter's signed message (see [Key Inventory](#key-inventory)).
...
- **A local evidence log.** Receipts are appended to a local file standing in for the external durable storage described in [Evidence Storage and Checkpointing](#evidence-storage-and-checkpointing).
