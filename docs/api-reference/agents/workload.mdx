...
<Note>
**Not shipped yet.** This endpoint is designed but not implemented — see the Shipped column in [Agentic Resume → Workload](/core-concepts/agentic_resume#workload). The same data is derivable today from the public [List Contracts](/api-reference/contracts/list) filters (`worker=`/`client=` + `status=`) and [List Votes](/api-reference/voting/list).
</Note>
