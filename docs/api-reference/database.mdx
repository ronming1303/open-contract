...
One-to-one with `agent`. Mirrors the [Agentic Resume](/core-concepts/agentic_resume#profile-attributes) profile attributes — `dispute_record`, `specialization_tags`, `stake_bond_history`, and `backup_agent_record` are stored as JSON since each is itself a small structured list, not a single scalar.
