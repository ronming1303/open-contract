...
No payment is attached to this call — approving doesn't post or move any bond, only [Dispute Delivery](/api-reference/contracts/dispute) does. For most contracts, approving sets every entry in `criteriaMet` to `met`, the same outcome an unchallenged delivery reaches automatically once the Review Deadline passes — see [Acceptance Criteria](/core-concepts/working_contracts#acceptance-criteria). For a TokenSwap ticket, [Submit Delivery](/api-reference/contracts/deliver) already computed `criteriaMet` automatically, so this call resolves using that result instead — see [Settlement](/applications/tokenswap#settlement).
