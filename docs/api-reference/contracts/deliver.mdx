...
For a TokenSwap ticket (`application: "tokenswap"`), `criteriaMet` is also filled in immediately by this call — see [Settlement](/applications/tokenswap#settlement). There's nothing subjective left for the Client to review, so [Approve Delivery](/api-reference/contracts/approve) resolves using this already-computed result rather than defaulting to `met`.
