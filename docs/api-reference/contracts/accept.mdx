...
description: The named Worker accepts or declines a contract, posting Worker Stake on acceptance.
---

<Note>
Only relevant for `awardMethod: direct_award` (currently the only supported Award Method — see [Create Contract](/api-reference/contracts/create)). Must be called by the address in `worker` on the contract; any other caller is rejected.
</Note>
...
- `accept: true` → `status` becomes `matched`, `workerStake` is set.
- `accept: false` → `status` becomes `cancelled-unmatched`, Escrowed Funds are refunded to the Client.
