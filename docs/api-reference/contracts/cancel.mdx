...
Cancelling moves the contract from `created` to `cancelled-by-initiator`. There's no stake to split here, unlike [Withdraw](/api-reference/contracts/withdraw) — no Worker has matched yet, so the Escrowed Funds are simply refunded to the Client in full.
