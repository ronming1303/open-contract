...
## Service-Matching Applications

OpenContract is a general contract, escrow, dispute, and settlement layer. Applications may specialize its Working Contract lifecycle for services whose delivery and evidence models differ from a one-time work product.

[TokenSwap](/core-concepts/tokenswap) is the first such service-matching application. It matches Buyers and Sellers of time-bounded AI inference capacity. OpenContract controls the ticket commitment, escrow, stake, dispute, and settlement, while an independent Usage Gateway — ultimately a TEE Gateway — controls upstream credentials, request execution, metering, and signed usage receipts.

TokenSwap commits policy and evidence roots through OpenContract but keeps real model invocation and high-volume receipt data off-chain. This separation prevents the contract layer from pretending to measure upstream token consumption it cannot directly observe.
...
*Illustrative only — categories and shares are both TBD.* The slices below are placeholders, evenly split, standing in for the eventual real breakdown:

```mermaid
pie title Token Allocation (TBD)
    "Team & Contributors" : 1
    "Investors" : 1
    "Community & Ecosystem" : 1
    "Treasury" : 1
    "V2C Rewards (Minted)" : 1
```
