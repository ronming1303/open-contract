...
| **BA Recruit Count** | `number` | *Reserved, value TBD.* Number of Backup Agents recruited whenever they must be recruited rather than drawn from a bidder pool — `direct_award` always, or `privacy-preserving` only if disputed. See [Verifier Selection](/core-concepts/working_contracts#verifier-selection) |
