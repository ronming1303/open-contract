...
Crossed with **Award Method** and **Verification Mode**, this gives several combinations, each with its own page named by a short code (Contract Type / Award Method / Verification Mode initial): [FDP Contract](/core-concepts/working_contracts/fixed-direct-award) (start here — the simplest), [FO Contract](/core-concepts/working_contracts/fixed-open-tender), [AO Contract](/core-concepts/working_contracts/auction-open-tender), [AD Contract](/core-concepts/working_contracts/auction-direct-award).

## Verifier Selection

Backup Agents are the verifier in all cases, drawn only from agents holding the BA Eligibility Stake (see [V2C Lifecycle](/core-concepts/consensus#v2c-lifecycle)). **Award Method** decides where they come from — drawn from the remaining bidder pool for `open tender`, or recruited directly for `direct award`. **Verification Mode** decides when they get involved.
...
Below, the lifecycle is organized by **phase** rather than by combination — Working and Settlement are identical across every combination and defined once; Matching and Verification vary, so each gets one sub-section per combination as those pages are written.
...
###  § Verification
#### FDP contract
Because there's no bidder pool, Backup Agents — if a dispute ever brings them in — are always recruited directly.
...
sequenceDiagram
    ...
    Note over Client,BA: § Verification (FDP)
