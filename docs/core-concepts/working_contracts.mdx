...
| **Verification Mode** | `enum` | `standard` — delivery is verified. `privacy-preserving` — Client reviews directly; materials are only decrypted to the verifier if a dispute is raised. Both currently shipped combinations (FDP, FOP — see [Lifecycle of Contract](#lifecycle-of-contract)) default to `privacy-preserving`: Backup Agents never get involved unless the Client disputes |
